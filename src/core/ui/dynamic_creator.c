...
static int32_t g_get_new_xy_ofs(g_obj *pg_obj, int32_t new_rot, int32_t par_w, int32_t par_h)
{
    ...
    LOG_INFO("ROTATE %d, X=%d - Y=%d", new_rot, real_x_ofs, real_y_ofs);

    return 0;
}


static int32_t g_obj_relocation(g_obj *pg_obj, int32_t rot)
{
    if (!pg_obj) {
        LOG_ERROR("Invalid g object");
        return -EINVAL;
    ...
    lv_obj_t *pl_par = NULL;
    pl_par = lv_obj_get_parent(pg_obj->obj);

    g_obj *pg_obj_par = NULL;
    pg_obj_par = pl_par->user_data;


    g_get_new_xy_ofs(pg_obj, rot, pg_obj_par->inf.w, pg_obj_par->inf.h);
...
static int32_t g_obj_resize(g_obj *pg_obj, int32_t rot)
{
    if (!pg_obj) {
        LOG_ERROR("Invalid g object");
        return -EINVAL;
...
static int32_t g_obj_rotate_relocation(g_obj *pg_obj, int32_t rot_dir)
{
    int32_t ret;
    int32_t tmp_val;
    ...
    if (pg_obj->inf.rot == rot_dir) {
        LOG_TRACE("Object is rotated already");
        return 0;
    } else {
        LOG_TRACE("Object is rotating to %d", rot_dir);
    }

    // Text, icon, switch will be rotate
    // Frame, button, slider will be resize and relocation

    // TODO: check return and obj type
    g_obj_resize(pg_obj, rot_dir);
    g_obj_relocation(pg_obj, rot_dir);

    pg_obj->inf.rot = rot_dir;
}
