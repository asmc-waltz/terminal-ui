...
static lv_obj_t *create_key(lv_obj_t *par, const key_def *key, kb_size_ctx *size)
{
    ...
    if (!lbl) {
        remove_obj_and_child_by_name(key->label, &(get_gobj_parent(par))->child);
        return NULL;
    }
...
int32_t pre_rotation_redraw_kb_layout(lv_obj_t *kb)
{
    ...
    set_gobj_align_scale_xy(kb, par, LV_ALIGN_BOTTOM_MID, 0, -KEYBOARD_PAD_BOT);
...
lv_obj_t *create_keyboard_containter(lv_obj_t *par)
{
    ...
    set_gobj_align_scale_xy(cont, par, LV_ALIGN_BOTTOM_MID, 0, -KEYBOARD_PAD_BOT);
