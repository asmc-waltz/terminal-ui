...
{
    lv_obj_t *lobj = gf_get_obj_by_name("screens.common", \
                            &((g_obj *)(lv_screen_active()->user_data))->child);

    g_set_scr_rot_dir(get_random_3());
...
{
    lv_obj_t *comm_page;
    comm_page = gf_get_obj_by_name("screens.common", &get_gobj(lv_screen_active())->child);
    if (!comm_page) {
...
lv_obj_t *create_scr_page(lv_obj_t *par, const char *name)
{
    ...
    lv_obj_t *sym_box = add_top_bar_symbol(top_space, TOP_BAR_NAME".wifi",
                                                ICON_WIFI_SOLID);
    gf_gobj_align_to(sym_box, top_space, LV_ALIGN_TOP_LEFT, TOP_BAR_SYM_ALN, 0);
...
#if defined(TEST)
    lv_obj_t *btn = gf_create_btn(lv_layer_top(), "common.rotate_btn");
